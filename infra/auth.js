...
exports.login = async (req, res) =>{
    try {
          ...
          connection.query('SELECT * FROM user WHERE usu_email = ?', [email], async (error, results) =>{
            console.log(results);
            
            if(!results || !(await bcrypt.compare(password, results[0].usu_key))){
...
exports.register = (req, res) => {
    ...
    console.log(req.body);
    connection.query('SELECT usu_email FROM user WHERE usu_email= ?',[email], async (error, results) => {
        if(error){
            console.log(error);
        }
        else if(name== ''|| email== ''){
        ...
        connection.query('INSERT INTO user SET ?', {usu_name: name, usu_email: email, usu_key: hashPwd}, (error, results) => {
            if(error){
                console.log(error);
            } else {
                console.log(results);
